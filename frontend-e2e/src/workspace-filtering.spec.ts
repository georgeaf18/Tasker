...
test.describe('Workspace Filtering', () => {
    ...
    test('should show only Work tasks when Work filter is active', async ({ page }) => {
        ...
        expect(await kanbanPage.hasTaskInColumn('Today', 'Personal Task 2')).toBe(false);
    });

    test('should show only Personal tasks when Personal filter is active', async ({ page }) => {
        // Create Work and Personal tasks
        await apiHelper.createTasks([
            { title: 'Work Task 1', workspace: 'WORK', status: 'BACKLOG' },
            { title: 'Work Task 2', workspace: 'WORK', status: 'IN_PROGRESS' },
            { title: 'Personal Task 1', workspace: 'PERSONAL', status: 'BACKLOG' },
            { title: 'Personal Task 2', workspace: 'PERSONAL', status: 'IN_PROGRESS' },
        ]);
        ...
        expect(await kanbanPage.hasTaskInColumn('In Progress', 'Personal Task 2')).toBe(true);

        // Verify Work tasks are NOT visible
        expect(await backlogPage.hasTask('Work Task 1')).toBe(false);
        expect(await kanbanPage.hasTaskInColumn('In Progress', 'Work Task 2')).toBe(false);
    });

    test('should show all tasks when no workspace filter is active', async ({ page }) => {
        // Create Work and Personal tasks
        await apiHelper.createTasks([
    ...
    test('should filter tasks across all statuses', async ({ page }) => {
        ...
        await apiHelper.createTasks([
            ...
            { title: 'Personal In Progress', workspace: 'PERSONAL', status: 'IN_PROGRESS' },
        ...
        expect(await kanbanPage.hasTaskInColumn('In Progress', 'Work In Progress')).toBe(true);
        ...
        expect(await kanbanPage.hasTaskInColumn('Today', 'Personal Today')).toBe(false);
        expect(await kanbanPage.hasTaskInColumn('In Progress', 'Personal In Progress')).toBe(false);
        expect(await kanbanPage.hasTaskInColumn('Done', 'Personal Done')).toBe(false);
    });
    ...
    test('should persist filter when creating new task', async ({ page }) => {
        ...
        await apiHelper.createTasks([
            ...
            { title: 'Existing Personal Task', workspace: 'PERSONAL', status: 'BACKLOG' },
        ]);
    ...
    test('should update daily progress based on filtered workspace', async ({ page }) => {
