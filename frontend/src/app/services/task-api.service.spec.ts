...
import { Task, CreateTaskDto, UpdateTaskDto, TaskFilters, Workspace, TaskStatus } from '../models';
...
describe('TaskApiService', () => {
    ...
    const createMockTaskResponse = (overrides?: Partial<any>): any => ({
        id: 1,
        title: 'Test Task',
        description: 'Test Description',
        workspace: Workspace.PERSONAL,
    ...
    describe('getTasks', () => {
        describe('without filters', () => {
            ...
            it('should deserialize all date fields correctly', (done) => {
                ...
                service.getTasks().subscribe({
                    next: (tasks) => {
                        ...
                        expect(tasks[0].createdAt.toISOString()).toBe('2024-01-01T00:00:00.000Z');
                        expect(tasks[0].dueDate?.toISOString()).toBe('2024-12-31T23:59:59.000Z');
                        done();
                    },
                    error: done.fail
                });
            ...
            it('should handle undefined channel correctly', (done) => {
                const mockResponse = [createMockTaskResponse({ channelId: null, channel: undefined })];
        ...
        describe('error handling', () => {
            it('should handle 400 Bad Request', (done) => {
                ...
                const req = httpMock.expectOne(baseUrl);
                req.flush({ message: 'Invalid workspace' }, { status: 400, statusText: 'Bad Request' });
            });
    ...
    describe('Error Message Customization', () => {
        it('should include backend error message in 400 response', (done) => {
            ...
            const req = httpMock.expectOne(baseUrl);
            req.flush({ message: 'Invalid workspace value' }, { status: 400, statusText: 'Bad Request' });
        });

        it('should fallback to generic message when no error message provided', (done) => {
            service.createTask({ title: 'Test', workspace: Workspace.PERSONAL }).subscribe({
                next: () => done.fail('Should have failed'),
                error: (error) => {
