...
describe('TaskStateService', () => {
  ...
  describe('loadTasks()', () => {
    ...
    it('should load tasks with filters', (done) => {
      ...
      const mockTasks = [
        createMockTask({ workspace: Workspace.WORK, status: TaskStatus.TODAY }),
      ];
      mockTaskApiService.getTasks.mockReturnValue(of(mockTasks));
    ...
    it('should handle error with no message', (done) => {
      mockTaskApiService.getTasks.mockReturnValue(throwError(() => ({})));
  ...
  describe('updateTaskStatus()', () => {
    it('should delegate to updateTask with status', (done) => {
      ...
      expect(mockTaskApiService.updateTask).toHaveBeenCalledWith(1, {
        status: TaskStatus.IN_PROGRESS,
      });
  ...
  describe('Complex Scenarios', () => {
    ...
    it('should handle filtering by both workspace and status', () => {
      ...
      const workTodayTasks = todayTasks.filter(
        (t) => t.workspace === Workspace.WORK,
      );
    ...
    it('should handle concurrent operations with loading state', () => {
      ...
      mockTaskApiService.getTasks.mockReturnValue(
        getTasksSubject.asObservable(),
      );
      mockTaskApiService.createTask.mockReturnValue(
        createTaskSubject.asObservable(),
      );
  ...
  describe('Signal Reactivity', () => {
    ...
    it('should trigger currentWorkspaceTasks update when workspace changes', () => {
      ...
      service.setSelectedWorkspace(Workspace.PERSONAL);
      expect(service.currentWorkspaceTasks()).toHaveLength(1);
      expect(service.currentWorkspaceTasks()[0].workspace).toBe(
        Workspace.PERSONAL,
      );
    });
  ...
  describe('Computed Signal: currentWorkspaceTodayTasks', () => {
    it('should return only TODAY tasks for selected workspace', () => {
      const tasks = [
        createMockTask({
          id: 1,
          status: TaskStatus.TODAY,
      ...
      const currentWorkspaceTodayTasks = service.currentWorkspaceTodayTasks();
      expect(currentWorkspaceTodayTasks).toHaveLength(2);
      expect(currentWorkspaceTodayTasks[0].id).toBe(1);
      expect(currentWorkspaceTodayTasks[1].id).toBe(4);
      expect(
        currentWorkspaceTodayTasks.every((t) => t.workspace === Workspace.WORK),
      ).toBe(true);
      expect(
        currentWorkspaceTodayTasks.every((t) => t.status === TaskStatus.TODAY),
      ).toBe(true);
    });
    ...
    it('should reactively update when workspace changes', () => {
      ...
      service.setSelectedWorkspace(Workspace.WORK);
      expect(service.currentWorkspaceTodayTasks()).toHaveLength(1);
      expect(service.currentWorkspaceTodayTasks()[0].id).toBe(1);

      service.setSelectedWorkspace(Workspace.PERSONAL);
      expect(service.currentWorkspaceTodayTasks()).toHaveLength(1);
      expect(service.currentWorkspaceTodayTasks()[0].id).toBe(2);
    });

    it('should return empty array when no TODAY tasks in selected workspace', () => {
      service.setTasks([
        createMockTask({
          status: TaskStatus.BACKLOG,
          workspace: Workspace.WORK,
      ...
      expect(service.currentWorkspaceTodayTasks()).toEqual([]);
    });
  });

  describe('Computed Signal: currentWorkspaceInProgressTasks', () => {
    it('should return only IN_PROGRESS tasks for selected workspace', () => {
      const tasks = [
        createMockTask({
          id: 1,
          status: TaskStatus.IN_PROGRESS,
      ...
      const currentWorkspaceInProgressTasks =
        service.currentWorkspaceInProgressTasks();
      expect(currentWorkspaceInProgressTasks).toHaveLength(2);
      expect(currentWorkspaceInProgressTasks[0].id).toBe(1);
      expect(currentWorkspaceInProgressTasks[1].id).toBe(4);
      expect(
        currentWorkspaceInProgressTasks.every(
          (t) => t.workspace === Workspace.WORK,
        ),
      ).toBe(true);
      expect(
    ...
    it('should reactively update when workspace changes', () => {
      ...
      service.setSelectedWorkspace(Workspace.WORK);
      expect(service.currentWorkspaceInProgressTasks()).toHaveLength(1);
      expect(service.currentWorkspaceInProgressTasks()[0].id).toBe(1);

      service.setSelectedWorkspace(Workspace.PERSONAL);
      expect(service.currentWorkspaceInProgressTasks()).toHaveLength(1);
      expect(service.currentWorkspaceInProgressTasks()[0].id).toBe(2);
    });

    it('should return empty array when no IN_PROGRESS tasks in selected workspace', () => {
      service.setTasks([
        createMockTask({ status: TaskStatus.TODAY, workspace: Workspace.WORK }),
        createMockTask({
          status: TaskStatus.IN_PROGRESS,
          workspace: Workspace.PERSONAL,
        }),
      ...
      expect(service.currentWorkspaceInProgressTasks()).toEqual([]);
    });
  });

  describe('Computed Signal: currentWorkspaceDoneTasks', () => {
    it('should return only DONE tasks for selected workspace', () => {
      const tasks = [
        createMockTask({
          id: 1,
          status: TaskStatus.DONE,
      ...
      const currentWorkspaceDoneTasks = service.currentWorkspaceDoneTasks();
      expect(currentWorkspaceDoneTasks).toHaveLength(2);
      expect(currentWorkspaceDoneTasks[0].id).toBe(1);
      expect(currentWorkspaceDoneTasks[1].id).toBe(4);
      expect(
        currentWorkspaceDoneTasks.every((t) => t.workspace === Workspace.WORK),
      ).toBe(true);
      expect(
        currentWorkspaceDoneTasks.every((t) => t.status === TaskStatus.DONE),
      ).toBe(true);
    });
    ...
    it('should reactively update when workspace changes', () => {
      ...
      service.setSelectedWorkspace(Workspace.WORK);
      expect(service.currentWorkspaceDoneTasks()).toHaveLength(1);
      expect(service.currentWorkspaceDoneTasks()[0].id).toBe(1);

      service.setSelectedWorkspace(Workspace.PERSONAL);
      expect(service.currentWorkspaceDoneTasks()).toHaveLength(1);
      expect(service.currentWorkspaceDoneTasks()[0].id).toBe(2);
    });

    it('should return empty array when no DONE tasks in selected workspace', () => {
      service.setTasks([
        createMockTask({ status: TaskStatus.TODAY, workspace: Workspace.WORK }),
        createMockTask({
          status: TaskStatus.DONE,
      ...
      expect(service.currentWorkspaceDoneTasks()).toEqual([]);
    });
  });

  describe('Error Handling Edge Cases', () => {
    it('should handle addTask error with no message property', (done) => {
      const dto: CreateTaskDto = {
        title: 'New Task',
        workspace: Workspace.PERSONAL,
      };
      mockTaskApiService.createTask.mockReturnValue(throwError(() => ({})));
    ...
    it('should handle updateTask error with no message property', (done) => {
      const task = createMockTask({ id: 1 });
      service.setTasks([task]);
      mockTaskApiService.updateTask.mockReturnValue(throwError(() => ({})));
    ...
    it('should handle removeTask error with no message property', (done) => {
      const task = createMockTask({ id: 1 });
      service.setTasks([task]);
      mockTaskApiService.deleteTask.mockReturnValue(throwError(() => ({})));
