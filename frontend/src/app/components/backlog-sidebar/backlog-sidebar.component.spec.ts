...
describe('BacklogSidebarComponent', () => {
  ...
  beforeEach(async () => {
    ...
    const backlogTasksSignal = signal<Task[]>(
    ...
    await TestBed.configureTestingModule({
      imports: [BacklogSidebarComponent, ReactiveFormsModule],
      providers: [
        provideHttpClient(),
  ...
  describe('Component Creation and Initialization', () => {
    ...
    it('should initialize with default form values', () => {
      ...
      expect(component.createTaskForm.get('workspace')?.value).toBe(
        Workspace.PERSONAL,
      );
      expect(component.createTaskForm.get('status')?.value).toBe(
        TaskStatus.BACKLOG,
      );
  ...
  describe('Computed Signal: backlogTasks()', () => {
    it('should return only tasks with BACKLOG status', () => {
      ...
      expect(backlogTasks.every((t) => t.status === TaskStatus.BACKLOG)).toBe(
        true,
      );
  ...
  describe('Computed Signals: workTaskCount() and personalTaskCount()', () => {
    ...
    it('should correctly count personal tasks', () => {
      ...
      const expectedPersonalTasks = mockTasks.filter(
        (t) =>
          t.status === TaskStatus.BACKLOG && t.workspace === Workspace.PERSONAL,
      );
      ...
      const personalOnlySignal = signal<Task[]>(
        mockTasks.filter(
          (t) =>
            ...
            t.workspace === Workspace.PERSONAL,
        ),
      );
      Object.defineProperty(mockTaskStateService, 'backlogTasks', {
  ...
  describe('Computed Signals: workTasks() and personalTasks()', () => {
    it('should return only work tasks', () => {
      ...
      expect(workTasks.every((t) => t.status === TaskStatus.BACKLOG)).toBe(
        true,
      );
    });
    ...
    it('should return only personal tasks', () => {
      ...
      expect(
        personalTasks.every((t) => t.workspace === Workspace.PERSONAL),
      ).toBe(true);
      expect(personalTasks.every((t) => t.status === TaskStatus.BACKLOG)).toBe(
        true,
      );
    });
  ...
  describe('Computed Signal: groupedByWorkspace()', () => {
    ...
    it('should group tasks by channel within workspace', () => {
      ...
      const frontendChannel = workGroup?.channels.find(
        (c) => c.channelName === 'Frontend',
      );
      expect(frontendChannel?.tasks.length).toBe(1);
      expect(frontendChannel?.tasks[0].id).toBe(1);

      const backendChannel = workGroup?.channels.find(
        (c) => c.channelName === 'Backend',
      );
    ...
    it('should handle tasks without channelId as "Uncategorized"', () => {
      ...
      const personalGroup = grouped.find(
        (g) => g.workspace === Workspace.PERSONAL,
      );
      const uncategorizedChannel = personalGroup?.channels.find(
        (c) => c.channelName === 'Uncategorized',
      );
    ...
    it('should handle missing channel data gracefully', () => {
      ...
      expect(
        workGroup?.channels.some(
          (c) => c.channel === null && c.tasks.some((t) => t.id === 99),
        ),
      ).toBe(true);
    });
  ...
  describe('Computed Signal: filteredChannels()', () => {
    ...
    it('should filter channels by workspace in create form', () => {
      ...
      expect(filtered.every((c) => c.workspace === Workspace.PERSONAL)).toBe(
        true,
      );
  ...
  describe('Task Details Dialog', () => {
    ...
    it('should populate edit form with task data', () => {
      ...
      expect(component.editTaskForm.get('description')?.value).toBe(
        testTask.description,
      );
      expect(component.editTaskForm.get('workspace')?.value).toBe(
        testTask.workspace,
      );
      expect(component.editTaskForm.get('channelId')?.value).toBe(
        testTask.channelId,
      );
      expect(component.editTaskForm.get('isRoutine')?.value).toBe(
        testTask.isRoutine,
      );
    });
  ...
  describe('Task Deletion', () => {
    ...
    it('should include task title in confirmation message', () => {
      ...
      expect(confirmationServiceInstance.confirm).toHaveBeenCalled();
      const confirmCall = (confirmationServiceInstance.confirm as jest.Mock)
        .mock.calls[0][0];
  ...
  describe('Integration Tests', () => {
    ...
    it('should handle complete task deletion flow', () => {
      ...
      const confirmCall = (confirmationService.confirm as jest.Mock).mock
        .calls[0][0];
      confirmCall.accept!();
  ...
  describe('Form Validation', () => {
    it('should mark createTaskForm as invalid when title is empty', () => {
      ...
      expect(component.createTaskForm.get('title')?.hasError('required')).toBe(
        true,
      );
    });
    ...
    it('should mark editTaskForm as invalid when title is empty', () => {
      ...
      expect(component.editTaskForm.get('title')?.hasError('required')).toBe(
        true,
      );
    });
